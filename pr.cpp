...
public:
  ...
  double getMaxFlow(int s, int t, bool verbose);
};
  ...
  for (int i = 0; i < edge.size(); i++) {
    ...
    if (edge[i].u == u) {
      ...
      if (ver[u].h > ver[edge[i].v].h) {
        ...
        double flow = min(edge[i].capacity - edge[i].flow, ver[u].e_flow);
...
double Graph::getMaxFlow(int s, int t, bool verbose) {
  preflow(s);
  ...
  if(verbose) {
    ...
    for (int i = 0; i < edge.size(); i++) {
      ...
      int v = edge[i].v;
      if(data[u][v] != 0) {
        printf("%d %d %f\n", u, v, edge[i].capacity - edge[i].flow);
      }
    }
  }
...
int main() {
  ...
  double max_flow = g.getMaxFlow(source, sink, verbose);
